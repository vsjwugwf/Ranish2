...
def handle_live_command(chat_id, text, url, need_scroll=False):
    ...
    if url.startswith("http://") or url.startswith("https://"):
        job = Job(job_id=str(uuid.uuid4()), chat_id=chat_id, mode="record_video", url=url, queue_type="record")
        job.extra = {"live_scroll": need_scroll}
        enqueue_job(job, "record")
...
def handle_callback(cq):
    ...
    elif data.startswith("scvid_"): enqueue_job(Job(job_id=str(uuid.uuid4()), chat_id=chat_id, mode="scan_videos", url="", queue_type="browser"), "browser")
    elif data.startswith("scdl_"): enqueue_job(Job(job_id=str(uuid.uuid4()), chat_id=chat_id, mode="scan_downloads", url="", queue_type="browser"), "browser")
    elif data.startswith("extcmd_"): enqueue_job(Job(job_id=str(uuid.uuid4()), chat_id=chat_id, mode="extract_commands", url="", queue_type="browser"), "browser")
    elif data.startswith("sman_"): enqueue_job(Job(job_id=str(uuid.uuid4()), chat_id=chat_id, mode="smart_analyze", url="", queue_type="browser"), "browser")
    elif data.startswith("srcan_"): enqueue_job(Job(job_id=str(uuid.uuid4()), chat_id=chat_id, mode="source_analyze", url="", queue_type="browser"), "browser")
